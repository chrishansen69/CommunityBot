...
const elizaData = require('./data/elizadata.js');
...
ElizaBot.prototype.reset = function() {
	...
	for (let k=0; k<elizaData.elizaKeywords.length; k++) {
		this.lastchoice[k]=[];
		let rules=elizaData.elizaKeywords[k][2];
...
ElizaBot.prototype._init = function() {
	...
	if ((elizaData.elizaSynons) && (typeof elizaData.elizaSynons == 'object')) {
		for (let i in elizaData.elizaSynons) synPatterns[i]='('+i+'|'+elizaData.elizaSynons[i].join('|')+')';
	}
	...
	// check for elizaData.elizaQuits and install default if missing
...
ElizaBot.prototype._execRule = function(k) {
	let rule=elizaData.elizaKeywords[k];
	let decomps=rule[2];
	let paramre=/\(([0-9]+)\)/;
